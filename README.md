...
# write (the engine writes — the assistant never hand-edits data.json)
python3 engine/gym.py log <session.json|->   # validate + append a session, recompute derived state
python3 engine/gym.py peso <kg> [--nota "..."]  # daily bodyweight
python3 engine/gym.py render-tracker         # regenerate tracker.md from data.json (never edited by hand)
python3 engine/gym.py selftest               # 25 checks guarding the engine — run after any change
```
...
2. **The engine decides, the assistant translates.** Next session = `gym.py next`. Targets = `gym.py compute`, copied **verbatim** — the assistant never re-words a prescription (re-wording is where drift is born).
   - **The engine also *writes*.** Sessions go in via `gym.py log`, bodyweight via `gym.py peso`, and `tracker.md` is fully generated by `gym.py render-tracker`. The assistant parses your messy input into a session JSON; the engine validates and writes it (and recomputes derived state). Nobody hand-edits `data.json` or `tracker.md` — that's where derived fields silently drift.
...
- **Weight jumps match your gym.** Increment defaults come from the equipment (`equip`: db/plate/barbell/smith → +2.5/side · cable/pin → +5). If a machine can't do the default jump — e.g. a plate-loaded calf with no 2.5 kg plates — set `"inc": 5` on that exercise and the engine will never propose an impossible jump. Each `equip` is also its own progression line, so switching dumbbell↔cable↔machine isn't read as a stall.
